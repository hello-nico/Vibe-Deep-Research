...
export function UploadedReports() {
  ...
  const upload = async () => {
    if (!file || !/^\d{6}$/.test(symbol)) { setError('请选择 PDF，并填写研报对应的 6 位 A 股代码'); return; }
    if (file.size > 32 * 1024 * 1024) { setError('研报不能超过 32 MB'); return; }
    setBusy(true); setError(''); setNotice('正在上传并整理研报，请稍候…');
    try {
      const report = await researchRead<Report>(`/documents/uploads?symbol=${symbol}&title=${encodeURIComponent(file.name)}`, { method: 'POST', headers: { 'Content-Type': 'application/pdf' }, body: file });
      setNotice(report.has_parsed && !report.extra.evidence_error ? '研报已就绪，可以开始研究。' : '原文件已保存，资料整理尚未完成，请查看下方处理状态。');
      await reload();
    } catch (e) { setError(String(e)); setNotice('上传结果未确认，可刷新列表核对；重传相同文件会按内容去重。'); }
    finally { setBusy(false); }
  };
  useAiPage({ key: 'uploaded-reports', title: '我的研报', context: '已上传研报元数据（本次未附正文）：' + JSON.stringify(items), suggestions: ['阅读研报时应如何区分事实和作者判断？'] });
  return <div><PageHeader title="我的研报" subtitle="收藏有价值的资料，让每一次阅读成为研究的起点" actions={<button className="workspace-action" disabled={busy} onClick={() => void reload().catch(e => setError(String(e)))}><RefreshCw size={14} />刷新列表</button>} />
    {/* 暂时隐藏研报归档入口，待明确用途后恢复。
    ...
    */}
    <GlassCard className="mb-6 !p-6"><div className="mb-5 flex items-center gap-3"><span className="rounded-xl bg-primary/10 p-3 text-primary"><Upload size={20} /></span><div><h2 className="font-semibold">添加研报</h2><p className="mt-1 text-xs text-muted-foreground">选择公司与 PDF 文件，开始整理你的研究资料</p></div></div>
      ...
      <label className="text-xs text-muted-foreground">公司代码<input className="workspace-field mt-2 block h-10 w-full" placeholder="例如 000933" value={symbol} disabled={busy} onChange={e => setSymbol(e.target.value.trim())} /></label>
      <div className="min-w-0"><div className="mb-2 text-xs text-muted-foreground">PDF 研报</div><label className="relative flex h-10 min-w-0 cursor-pointer items-center gap-2 rounded-xl border border-border bg-background/50 px-3 transition-colors hover:border-primary/50 focus-within:ring-2 focus-within:ring-primary">
        ...
        <input className="sr-only" type="file" accept="application/pdf,.pdf" aria-label="选择 PDF 研报" disabled={busy} onChange={e => setFile(e.target.files?.[0] ?? null)} />
      </label></div>
      <button className="workspace-action workspace-action-primary" disabled={busy || !file || !/^\d{6}$/.test(symbol)} onClick={() => void upload()}><Upload />{busy ? '正在整理…' : '上传研报'}</button></div>
      <p className="mt-3 text-xs text-muted-foreground">支持 A 股公司研报，单个 PDF 不超过 32 MB。上传后可查看原文并继续提问。</p>
      {notice && <p role="status" className="mt-2 text-sm">{notice}</p>}{error && <p role="alert" className="mt-2 text-sm">{error}</p>}
    </GlassCard>
