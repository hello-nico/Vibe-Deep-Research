...
export function Backtest() {
  ...
  useAiPage({
    key: "backtest-agent", title: "回测助手",
    context: lastReport || "回测助手：先通过对话厘清假设与规则，再调用真实回测工具，完成后自动生成并归档报告。",
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="回测" subtitle="把想验证的假设直接告诉助手。它会补齐必要条件、选择工具、执行回测，并自动保存完整报告。"
      ...
      <GlassCard className="ai-surface !p-0 overflow-hidden border-primary/25">
        <div className="ai-surface-header flex items-center gap-3 border-b border-border/60 px-5 py-4">
          ...
          <p className="text-sm font-semibold">回测助手</p>
        </div>
        ...
        <div className="min-h-[360px] max-h-[620px] space-y-5 overflow-y-auto px-5 py-5">
          {messages.length === 0 && <div className="mx-auto flex max-w-xl flex-col items-center py-16 text-center">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">可以从一句不完整的想法开始。助手会判断还缺哪些标的、时间范围、规则或对照，再继续追问。</p>
          </div>}
          ...
          {running && <div className="flex items-center gap-3 text-sm text-muted-foreground">
            ...
            助手正在判断信息是否齐全；条件齐了以后会自动获取数据并逐条回测…
          </div>}
          <div ref={bottomRef} />
        </div>
