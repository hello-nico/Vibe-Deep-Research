...
export function IndustryProfiles() {
  ...
  useAiPage({ key: `industry-profile:${key ?? 'list'}`, title: profile?.industry_name || '板块中心', context: profile ? JSON.stringify(profile) : '申万行业 Profile 目录，尚未选定行业。', suggestions: ['这个行业最值得核实的问题是什么？'] });
  return <div><PageHeader title={profile?.industry_name || '板块中心'} subtitle="沿着产业脉络，发现值得深入研究的问题" />
    <div className="workspace-toolbar">{key && <Link className="workspace-action" to="/sectors"><ChevronLeft />行业目录</Link>}<Link className="workspace-action" to="/sectors/legacy"><Layers3 />产业链专题</Link></div>
    ...
    {items && <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{items.map(item => {
      ...
      return item.status === 'ready' ? <Link className="group" key={item.industry_code} to={`/sectors/${item.industry_code}`}>{card}</Link> : <div key={item.industry_code}>{card}</div>;
    })}</div>}
