...
export function ResultCard({ payload, sourceKey, presentation = 'conversation' }: { payload: Result['payload']; sourceKey: string; presentation?: 'conversation' | 'report' }) {
  ...
  const option = useMemo(() => {
    ...
    if (payload?.kind === 'financial') return {
      ...
      series: (payload.series ?? []).map(key => ({ name: payload.columns.find(column => column.key === key)?.label ?? key,
    ...
    return {
      ...
      yAxis: [{ scale: true, name: '元' }, { gridIndex: 1, scale: true, name: payload?.columns.find(c => c.key === 'volume')?.unit }],
  ...
  return <section className="my-4 w-full min-w-0 rounded-2xl border bg-background p-5" aria-label={payload.title}>
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex gap-3"><button aria-pressed={!table} onClick={() => setTable(false)}>图表</button><button aria-pressed={table} onClick={() => setTable(true)}>数据</button><button onClick={() => {
        setDownloadError('');
        if (table) downloadFile(payload.title + '.csv', new Blob([dataCsv(payload.columns, payload.rows)], { type: 'text/csv;charset=utf-8' }));
          ...
          void downloadChart(image, payload.title, [
            ...
            `来源：${[...new Set(payload.sources.map(source => sourceName(source.title)))].join('、')} · 获取时间 ${new Date(payload.fetched_at).toLocaleString()}`,
    ...
    {table ? <div className="mt-4 max-h-[60vh] overflow-auto"><table className="w-full whitespace-nowrap text-right text-sm tabular-nums"><thead><tr>{payload.columns.map(column => <th className="p-2" key={column.key}>{column.label}{column.unit && `（${column.unit}）`}</th>)}</tr></thead><tbody>{rows.map(row => <tr className="border-t" key={row.trading_day ?? row.period}>{payload.columns.map(column => <td className="p-2" key={column.key}>{financialNumber(row[column.key], column.unit === '股' ? 0 : column.unit ? 2 : undefined)}</td>)}</tr>)}</tbody></table></div> : payload.kind === 'market' && payload.chart === 'candlestick'
      ? <MarketChart rows={rows} title={payload.title} onReady={readyMarket} />
      : presentation === 'report' && payload.kind === 'financial'
        ? <ReportFinancialChart rows={rows} series={payload.series ?? []} columns={payload.columns} onReady={readyMarket} />
        : <EChart option={option} height={400} onReady={readyEChart} />}
    {payload.missing.map(gap => <p className="text-sm text-muted-foreground" key={gap}>{gap}</p>)}
