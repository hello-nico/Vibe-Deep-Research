...
function PanelChat({ sessionId, topic, assistant, useSession, useConversation, useChat, useSessions, inputActions, renderFactorySlot }: PropsRuntime<'finance.panel.chat'> & PropsRenderFactories) {
  ...
  return <div className={topic ? 'finance-topic-chat' : assistant ? 'finance-assistant-native' : 'finance-panel-chat'}>
...
export function installPanelConversation(ctx: Context, activePanel: () => FinanceSidePanel | null) {
  ...
  function SaveAssistantTurn({ sessionId, turn, useChat }: PropsRuntime<'conversation.chat.turnTail'>) {
    const usersSource = useChat(value => value.nodes.turnDataSource(turn.turn, 'user'));
    const users = useSyncExternalStore(usersSource.subscribe, usersSource.getSnapshot, usersSource.getSnapshot);
    const errorsSource = useChat(value => value.nodes.turnDataSource(turn.turn, 'turn-error'));
    const errors = useSyncExternalStore(errorsSource.subscribe, errorsSource.getSnapshot, errorsSource.getSnapshot);
    const tailSource = turn.data.source('turn-tail');
    const tail = useSyncExternalStore(tailSource.subscribe, tailSource.getSnapshot, tailSource.getSnapshot);
    const note = assistantTurnNote({ panel: activePanel(), sessionId, turnClosed: turn.status === 'closed',
      failed: errors.length > 0, users, closing: tail?.closing });
    return note && <SaveNoteButton kind="问助手" title={note.title} content={note.content} />;
  }
  ctx.slots.inject('conversation.chat.turnTail', () => ctx.slots.register({
    name: 'conversation.chat.turnTail', id: 'finance-assistant-save', order: 60,
  }, SaveAssistantTurn));
}
