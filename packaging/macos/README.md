...
当前产品仅交付 Web，以下内容仅为历史记录，不是安装指引、验收剩余项或 M9 待办。
...
M34 将 M33 的“启动后打开浏览器”改为独立 AppKit + WKWebView 窗口，继续使用现有 App 图标。现有页面与导航不变。官方 Codex SDK 与捆绑引擎锁定 0.153.4，零 fork。
...
下文保留原生窗口与组包的历史技术说明，不代表当前安装包可交付。
旧 SDK 执行链已经退役，构建入口在写文件和下载依赖前明确退出。
当前使用仓库根的 `scripts/start` 启动 DSH 工作台。
不再安排原生客户端接通或独立 App 验收。
