 ...
 * 另两种在闭环之外曾用台账保存：`watch`（自选）与 `note`（研究记录）。
 * M8.5 后自选改走产品 SQLite，沉淀记录改走 Backend；台账种类保留以便读备份，
 * 产品服务写入会被拒绝。
 *
...
export const FINANCE_LEDGER_KINDS: Record<string, LedgerKindDef> = {
   ...
   *
   * M8.5 后产品写入走 Client SQLite；此种类仅保留备份读取与字段校验。
   *
   ...
   *
   * M8.5 后正文与身份归 Backend `product_notes`；此种类仅保留备份读取。
   *
